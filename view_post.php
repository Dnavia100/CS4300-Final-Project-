...
<body>
   ...
   <div class="content">
        ...
        while($row = $res->fetch_assoc()) {
            ...
            <div style="min-height: 50px;">'.$row["post_title"]."<br> by ".$row["post_creator"].' - '.$row["post_date"].'<hr>'.$row["post_content"].'</div></td></tr>
            <tr><td colspan="2"><hr></td></tr>';
            $sql2 = "SELECT * FROM Replies WHERE Parent_ID='".$cid."' ORDER BY Reply_Date ASC";
            $res2 = $conn->query($sql2) or die($conn->error);
            while($row2 = $res2->fetch_assoc()) {
                echo '<tr><td valign="top" style="border: 1 px solid #000000;">
                <div style="min-height: 50px;">'.$row2["Reply_Creator"].' - '.$row2["Reply_Date"].'<hr>'.$row2["Reply_Content"].'</div></td></tr>
                <tr><td colspan="2"><hr></td></tr>';
            }
        }
